        ...
        with ExitStack() as stack:
            ...
            with accelerator.autocast():
                ...
                if spec_align_heads is not None:
                    ...
                    patches_view = accelerator.unwrap_model(student).extract_patches_view(x)  # (B,C,P,S)
                ...
                if rel_z_projector is not None:
                    ...
                    patches_view = accelerator.unwrap_model(student).extract_patches_view(x)
                    c_safe = c_tgt.clamp(min=0)
                    t_safe = t_tgt.clamp(min=0)
                    B_, Lt = c_safe.shape
                    b_idx = torch.arange(B_, device=x.device)[:, None].expand(B_, Lt)
        ...
        if do_step:
            ...
            if accelerator.is_main_process and (global_step % train_cfg.log_every == 0):
                ...
                logs = {
                    ...
                    "spatial_emb_scale": float(accelerator.unwrap_model(student).coord_embed.emb_w.detach().cpu().item()),
                    "spec_loss": spec_loss,
                    "spec_lam": spec_lam,
                }
            ...
            # if accelerator.is_main_process and (global_step % train_cfg.save_every == 0):
                ...
                if accelerator.is_main_process:
                    accelerator.unwrap_model(student).save_pretrained(os.path.join(ckpt_dir, f"student_{global_step+1:07d}"))
                    torch.save(accelerator.unwrap_model(predictor).state_dict(), os.path.join(ckpt_dir, f"predictor_{global_step+1:07d}.pt"))
                    accelerator.unwrap_model(teacher).save_pretrained(os.path.join(ckpt_dir, f"teacher_{global_step+1:07d}"))
